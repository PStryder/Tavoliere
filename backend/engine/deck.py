...
def create_deck(recipe: DeckRecipe) -> list[Card]:
    ...
    if recipe == DeckRecipe.STANDARD_52:
        return _make_cards(list(Rank), _ALL_SUITS, copies=1)
    elif recipe == DeckRecipe.EUCHRE_24:
        return _make_cards(_EUCHRE_RANKS, _ALL_SUITS, copies=1)
    elif recipe == DeckRecipe.DOUBLE_PINOCHLE_80:
        return _make_cards(_PINOCHLE_RANKS, _ALL_SUITS, copies=4)
    else:
        raise ValueError(f"Unknown deck recipe: {recipe}")


def _make_cards(ranks: list[Rank], suits: list[Suit], copies: int) -> list[Card]:
    cards = []
